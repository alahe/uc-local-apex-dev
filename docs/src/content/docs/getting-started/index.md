...
:::tip[New to the team?]
If you're setting this up for the first time as part of onboarding, check the [Onboarding Checklist](/products/uc-local-apex-dev/docs/getting-started/onboarding/) first — it covers hardware/software prerequisites, access requirements, and day-to-day usage in one place.
:::
...
These requirements differ enough by platform (and by what your organization allows you to install directly)
that it's worth checking the [Onboarding Checklist → Required Software](/products/uc-local-apex-dev/docs/getting-started/onboarding/#2-required-software)
for the Windows/macOS/Linux breakdown — including a Podman Desktop-only path that covers both the container
engine and Compose, and a VS Code extension alternative for SQLcl if direct downloads are blocked.
...
<Aside type="caution" title="Required for Windows">
Install [Podman Desktop](https://podman-desktop.io/) **first**, before continuing with the rest of this guide.
It bundles the Podman CLI and automatically creates and configures the WSL2 machine used to run containers —
by far the fastest way to get a working environment on Windows.
</Aside>

We recommend using WSL2 (Windows Subsystem for Linux) to run this project's bash scripts. Follow these guides:
...
Podman Desktop manages its own WSL2 distro for the container engine — you don't need to separately install
Podman or Docker inside your Ubuntu-on-WSL2 distro; run this project's scripts there and they'll reach the
Podman machine over its API socket. Podman Desktop also covers the Compose requirement (`podman compose`),
so there's nothing extra to install for that either.

If direct SQLcl downloads from `download.oracle.com` are blocked in your organization, see the
[Onboarding Checklist](/products/uc-local-apex-dev/docs/getting-started/onboarding/#2-required-software) for
a VS Code extension alternative that bundles both SQLcl and a compatible JDK.

<Aside type="note" title="Prefer Docker instead?">
If your team standardizes on Docker instead of Podman, [install Docker in Ubuntu](https://docs.docker.com/engine/install/ubuntu/)
inside WSL2 instead — Podman Desktop is not required in that case, but setup takes longer since Docker's own
WSL2 integration must then be configured manually.
</Aside>

<Aside type="note" title="Using an internal company mirror?">
If your team pulls ADB images through an internal Artifactory or mirrored registry, start with the generic flow in [ADB Free](/products/uc-local-apex-dev/docs/getting-started/adb-free/) and copy the sanitized private setup template from [Enterprise Mirror Setup Template](/products/uc-local-apex-dev/docs/getting-started/enterprise-mirror-template/) into your internal wiki or private repository.
</Aside>
...
3. **Understand how it all fits together**: see the [Architecture Overview](/products/uc-local-apex-dev/docs/reference/architecture/) for containers, volumes, and where passwords live
4. **Start developing your APEX applications**
...
| `FORCE_SECURE` | Set to `true` to auto-generate a self-signed cert and enable HTTPS on port 8443 during `install.sh` | `install.sh`, `scripts/create-self-signed-certificates.sh` |
